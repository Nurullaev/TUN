...
| Команда | Доступ | Описание | Детали |
| :--- | :---: | :--- | :--- |
| `/help` | 👥 Все | Список команд | Показывает это меню помощи |
| `/key` | 👥 Все | AES ключ для клиента | Ключ скрыт спойлером, кликните для просмотра |
| `/status` | 🔐 Админ | Статус VK-туннеля | PID, uptime, последняя проверка |
| `/restart-tunnel` | 🔐 Админ | Перезапуск туннеля | Останавливает и запускает vk-tunnel заново |
| `/restart-server` | 🔐 Админ | Перезапуск server.py | kill -9 → nohup python server.py |
| `/log` | 🔐 Админ | Последние логи | Выводит 20 последних строк из manager.log |

**Легенда:** 👥 - доступно всем | 🔐 - только администратору
